...
The name or ID of a container that should be sent a signal when the template is changed.
The target does not need to match the proxytag filter.
...
passed in using the `DOTEGE_PROXYTAG` env var. This filter only applies to template inputs,
not the container specified in `DOTEGE_SIGNAL_CONTAINER`.
